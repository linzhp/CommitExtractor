 ...
 * 'BugFixMessage','Patches','Hunks','HunkBlame','FileTypes','Content',
 * 'CommitsLOC'
 * 
 */
...
public class Extractor {
	...
	/**
	 * @param args
	 *            args[0] is the configuration file name
	 */
	public static void main(String[] args) throws Exception {
		...
		ResultSet commitRS = stmt
				.executeQuery("select s.id as commit_id, author_id, author_date, rev, message "
						+ "from scmlog s"
						+ " where s.repository_id in("
						+ repoID + ") order by author_date");
		...
		while (commitRS.next()) {
			...
			BagOfWordsPlus aDeltaBOW = new BagOfWordsPlus("added_delta"), dDeltaBOW = new BagOfWordsPlus(
					"deleted_delta");
			...
			while (file.next()) {
				...
				newRevLOC += extractSrcFeatures(commit, contentBOW, aDeltaBOW,
						dDeltaBOW, file);
			...
			features.put(getOrCreateIndex("files_copied"),
					commit.getFilesCopied());
			...
			features.put(
					getOrCreateIndex("author#" + commitRS.getInt("author_id")),
					1);
			features.put(getOrCreateIndex("cumulative_change_count"),
					cumulative_change_count);
			features.put(getOrCreateIndex("cumulative_bug_count"),
					cumulative_bug_count);
			...
			for (String term : termFreq.keySet()) {
				...
				if (cumulative_change_count <= Integer.valueOf(prop
						.getProperty("VocabularyRev"))) {
	...
	private static int extractSrcFeatures(Commit commit,
			BagOfWordsPlus contentBOW, BagOfWordsPlus aDeltaBOW,
		...
		ResultSet rs = stmt.executeQuery("select patch from patches "
				+ "where commit_id=" + commit.getID() + " and file_id="
				+ fileID);
		if (rs.next()) {
	...
	private static void extractASTDelta(Commit commit, ResultSet file)
		...
		switch (actionType) {
		case 'C':
			commit.incrFilesCopied();
			break;
		case 'M':
			commit.processModify(fileID);
			break;
		case 'D':
			commit.processDelete(fileID);
			break;
		case 'A':
			commit.processAdd(fileID);
			break;
		case 'V':
			commit.processRename(fileID);
			break;
		}
	}

	public static Set<String> getFixeRevs(int bug_commit_id)
			throws SQLException {
		Set<String> revs = new TreeSet<String>();
		Connection conn = DatabaseManager.getConnection();
		Statement stmt = conn.createStatement();
		ResultSet rs = stmt
				.executeQuery("select rev, is_bug_fix "
